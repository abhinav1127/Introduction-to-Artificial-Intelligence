...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i, weight in enumerate(self.weights):
            self.weights[i] += (alpha * inActs[i] * delta)
            totalModification += abs(alpha * inActs[i] * delta)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            inActs = [percep.sigmoidActivation(inActs) for percep in layer]
            outputs.append(inActs)
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    error = 0.0
                    for index, nextNeuron in enumerate(nextLayer):
                        Deltatemp = nextNeuron.weights[neuronNum + 1] * deltas[0][index]
                        error += Deltatemp
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    iteration=0
    trainError=0
    weightMod= weightChangeThreshold + 100
    ...
    for features, result in examplesTest:
        output = nnet.feedForward(features)

        correct = True
        for index, node in enumerate(output[-1]):
            if round(output[-1][index]) != result[index]:
                correct = False
        if correct:
            testCorrect += 1
        else:
    ...
    testAccuracy= float(testCorrect / float(testError + testCorrect))#num correct/num total
